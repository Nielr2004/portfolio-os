...
import BootScreen from '@/components/os/BootScreen'; // <-- NEW IMPORT
import LockScreen from '@/components/os/LockScreen'; // <-- NEW IMPORT

// --- CUSTOM WIDGETS (FileManagerWidget kept for completeness) ---
...
const coreApps = [
  { id: 'files', title: 'File Manager', icon: <Folder size={26} />, bg: 'bg-blue-500', widget: <FileManagerWidget /> },
  { id: 'about', title: 'Profile', icon: <User size={26} />, bg: 'bg-zinc-800', widget: <ProfileWidget /> },
  { id: 'work', title: 'Work', icon: <Chrome size={26} />, bg: 'bg-zinc-700', widget: <ExperienceWidget /> },
  { id: 'skills', title: 'Skills', icon: <Settings size={26} />, bg: 'bg-purple-600', widget: <SkillsWidget /> },
];
...
const socialApps = [
  ...
  { id: 'instagram', title: 'Instagram', icon: <Instagram size={26} />, bg: 'bg-gradient-to-br from-pink-500 to-violet-600', url: 'https://www.instagram.com/_.roybabu._/' },
];
...
export default function Home() {
  ...
  // NEW STATE: Manages the application lifecycle
  ...
  // --- MOBILE HANDLERS (for touch area) ---
  ...
  return (
    <main 
      ...
      // Apply wallpaper only if not mobile (mobile handles its own background)
    ...
    >
      {/* --- OS LIFECYCLE (Boot and Lock Screens) --- */}
      <AnimatePresence>
            ...
            >
                ...
                <>
                  ...
                  <div className="absolute inset-0 top-8 z-0">
                     ...
                     <div className="absolute left-8 top-4 flex flex-col gap-8">
                         {/* Profile Photo - Increased size to w-32 for prominence */}
              ...
              {isMobile && (
                <>
                  ...
                  {/* Dock Area (Mobile - Floating Glass Dock) */}
                  <AnimatePresence>
                     {currentPage === 0 && (
                        <motion.div initial={{ y: 100 }} animate={{ y: 0 }} exit={{ y: 100 }} className="w-full px-4 pb-4 z-30 flex flex-col gap-5">
                           {/* Search (Floating Glass Pill - Tap to Open Drawer) */}
                           <div 
                               onClick={() => setCurrentPage(1)}
                               className="w-full h-[52px] bg-white/5 backdrop-blur-xl rounded-full flex items-center px-5 gap-3 shadow-2xl border border-white/10 active:scale-95 transition-transform cursor-pointer"
                           >
                           ...
                           {/* Dock Icons (Floating Glass Dock) */}
                  ...
                  {/* NEW: GESTURE INDICATOR BAR (Replaces 3-Button Nav) */}
                  <div className="h-6 w-full flex items-center justify-center shrink-0 z-50 bg-black/90 backdrop-blur-lg">
                      <div className="w-32 h-1 bg-white/40 rounded-full" /> 
                  </div>

                  {/* 3-BUTTON NAV (Removed, but retained handlers for Back/Home/Recents if needed) */}
                  {/* Note: In the new design, the bottom gesture bar handles 'Home/Recents' via swipe up/down. Back is handled in apps/windows. */}
